...
public class AIElementFilterEventHandler : IExternalEventHandler, IWaitableExternalEventHandler
{
    ...
    public void Execute(UIApplication uiapp)
    {
        ...
        try
        {
            ...
            // 检查过滤器设置是否有效
            if (!FilterSetting.Validate(out var errorMessage))
    ...
    private static List<Element> GetElementsByKind(Document doc, FilterSetting settings, bool isElementType, List<string> appliedFilters)
    {
        ...
        if (!string.IsNullOrWhiteSpace(settings.FilterElementType))
        {
            ...
            // 尝试解析类型名称的各种可能形式
            var possibleTypeNames = new string[]
            {
        ...
        {
            var symbolId = new ElementId(settings.FilterFamilySymbolId);
            // 检查元素是否存在且是族类型
            var symbolElement = doc.GetElement(symbolId);
            if (symbolElement != null && symbolElement is FamilySymbol)
            {
                var familyFilter = new FamilyInstanceFilter(doc, symbolId);
                filters.Add(familyFilter);
                // 添加更详细的族信息日志
                var symbol = symbolElement as FamilySymbol;
                var familyName = symbol.Family?.Name ?? "未知族";
                var symbolName = symbol.Name ?? "未知类型";
                appliedFilters.Add($"族类型：{familyName} - {symbolName} (ID: {settings.FilterFamilySymbolId})");
            }
            else
            {
                var elementType = symbolElement != null ? symbolElement.GetType().Name : "不存在";
        ...
        if (settings.BoundingBoxMin != null && settings.BoundingBoxMax != null)
        {
            ...
            var minXYZ = JZPoint.ToXYZ(settings.BoundingBoxMin);
            var maxXYZ = JZPoint.ToXYZ(settings.BoundingBoxMax);
            // 创建空间范围Outline对象
    ...
    public static ElementInstanceInfo CreateElementFullInfo(Document doc, Element element)
    {
        try
        {
            ...
            var elementInfo = new ElementInstanceInfo();        //创建存储元素完整信息的自定义类
            // ID
            ...
            // 最大包围盒
            var boundingBoxInfo = new BoundingBoxInfo();
            elementInfo.BoundingBox = GetBoundingBoxInfo(element);
            // 参数
            //elementInfo.Parameters = GetDimensionParameters(element);
            var thicknessParam = GetThicknessInfo(element);      //厚度参数
            if (thicknessParam != null)
            {
                elementInfo.Parameters.Add(thicknessParam);
            }
            var heightParam = GetBoundingBoxHeight(elementInfo.BoundingBox);      //高度参数
            if (heightParam != null)
            {
    ...
    public static ElementTypeInfo CreateTypeFullInfo(Document doc, ElementType elementType)
    {
        ...
        var thicknessParam = GetThicknessInfo(elementType);      //厚度参数
        if (thicknessParam != null)
        {
    ...
    public static PositioningElementInfo CreatePositioningElementInfo(Document doc, Element element)
    {
        try
        {
            if (element == null)
                return null;
            var info = new PositioningElementInfo
            {
    ...
    public static SpatialElementInfo CreateSpatialElementInfo(Document doc, Element element)
    {
        try
        {
            if (element == null || !(element is SpatialElement))
                return null;
            var spatialElement = element as SpatialElement;
            var info = new SpatialElementInfo
            {
                Id = element.Id.IntegerValue,
                UniqueId = element.UniqueId,
                Name = element.Name,
            ...
            // 获取周长
            var perimeterParam = element.get_Parameter(BuiltInParameter.ROOM_PERIMETER);
    ...
    public static AnnotationInfo CreateAnnotationInfo(Document doc, Element element)
    {
        try
        {
            ...
            // 获取所在视图
            var viewParam = element.get_Parameter(BuiltInParameter.VIEW_NAME);
        ...
        try
        {
            ...
            else if (element is RevitLinkInstance linkInstance)
            {
                ...
                if (linkType != null)
                {
                    ...
                    // 获取绝对路径
                    var absPath = ModelPathUtils.ConvertModelPathToUserVisiblePath(extFileRef.GetAbsolutePath());
                ...
                // 获取位置
                var location = linkInstance.Location as LocationPoint;
                if (location != null)
                {
    ...
    public static ElementBasicInfo CreateElementBasicInfo(Document doc, Element element)
    {
        try
        {
            if (element == null)
                return null;
            var basicInfo = new ElementBasicInfo
            {
                Id = element.Id.IntegerValue,
                UniqueId = element.UniqueId,
                Name = element.Name,
    ...
    public static ParameterInfo GetThicknessInfo(Element element)
    {
        ...
        // 获取构件类型
        var elementType = element.Document.GetElement(element.GetTypeId()) as ElementType;
        if (elementType == null)
        {
            return null;
        }
    ...
    public static LevelInfo GetElementLevel(Document doc, Element element)
    {
        try
        {
            ...
            {
                // 尝试获取族实例的标高参数
                var levelParam = familyInstance.get_Parameter(BuiltInParameter.FAMILY_LEVEL_PARAM);
                if (levelParam != null && levelParam.HasValue)
                {
                    level = doc.GetElement(levelParam.AsElementId()) as Level;
                }
    ...
    public static ParameterInfo GetBoundingBoxHeight(BoundingBoxInfo boundingBoxInfo)
    {
        try
        {
            ...
            // Z轴方向的差值即为高度
            var height = Math.Abs(boundingBoxInfo.Max.Z - boundingBoxInfo.Min.Z);
...
#else
            // 判断参数是否为尺寸相关的类型
            var isDimensionType = param.Definition.ParameterType == ParameterType.Length ||
