...
public class CreateSurfaceElementEventHandler : IExternalEventHandler, IWaitableExternalEventHandler
{
            ...
            foreach (var data in CreatedInfo)
            {
                ...
                switch (builtInCategory)
                {
                    case BuiltInCategory.OST_Floors:
                        if (floorType == null)
                        {
                            using (var transaction = new Transaction(doc, "创建楼板类型"))
                            {
                                transaction.Start();
                ...
                // Step3 批量创建楼板
                Floor floor = null;
                using (var transaction = new Transaction(doc, "创建面状构件"))
                {
                    transaction.Start();
                    ...
                    switch (builtInCategory)
                    {
                        case BuiltInCategory.OST_Floors:
                            ...
                            var curveLoop = CurveLoop.Create(data.Boundary.OuterLoop.Select(l => JZLine.ToLine(l) as Curve).ToList());
    ...
    private FloorType CreateOrGetFloorType(Document doc, double thickness = 200 / 304.8)
    {
        ...
        if (existingType != null)
            return existingType;
        // 如果没有找到匹配的楼板类型，创建新的
        var baseFloorType = existingType = new FilteredElementCollector(doc)
            .OfClass(typeof(FloorType))                    // 仅获取FloorType类
            .OfCategory(BuiltInCategory.OST_Floors)        // 仅获取楼板类别
            .Cast<FloorType>()                            // 转换为FloorType类型
        ...
        // 设置新楼板类型的厚度
        // 获取构造层设置
        var cs = newFloorType.GetCompoundStructure();
        if (cs != null)
        {
            // 获取所有层
            ...
            {
                // 计算当前总厚度
                var currentTotalThickness = cs.GetWidth();

                // 按比例调整每层厚度
                for (var i = 0; i < layers.Count; i++)
                {
                    var layer = layers[i];
                    var newLayerThickness = thickness;
